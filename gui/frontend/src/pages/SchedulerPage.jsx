...
function DailyCaptureProgress({ progress }) {
  ...
  return <div className="daily-progress"><div className="daily-progress-heading"><div><h4>{progress.is_today ? "Today’s imaging progress" : "Daily imaging progress"}</h4><p>{dateLabel} · Select a successful marker with a saved image to inspect it. Smaller markers show technical replicates.</p></div><div className="daily-progress-legend"><span className="succeeded">Successful</span><span className="failed">Failed</span><span className="missed">Missed</span><span className="pending">Pending</span><span className="remaining">Remaining</span></div></div><ScrollableProgressTimeline points={points} ticks={ticks} next={next} completedWidth={completedWidth} pulseStart={pulseStart} pulseWidth={pulseWidth} pointerAlignment={pointerAlignment} onSelect={setSelected} />{selected && <CaptureImageModal capture={selected} onClose={() => setSelected(null)} />}</div>;
}

function CaptureImageModal({ capture, onClose }) {
  const source = `/api/scheduler/capture-image?scheduled_at=${encodeURIComponent(capture.scheduled_at)}`;
  return createPortal(<div className="capture-image-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><section className="capture-image-modal" role="dialog" aria-modal="true" aria-labelledby="capture-image-title"><header><div><span>Captured image</span><h3 id="capture-image-title">{capture.time} · Replicate {capture.replicate}</h3></div><button type="button" aria-label="Close captured image" onClick={onClose}>×</button></header><img src={source} alt={`Capture taken at ${capture.time}, replicate ${capture.replicate}`} /></section></div>, document.body);
}
...
function ScrollableProgressTimeline({ points, ticks, next, completedWidth, pulseStart, pulseWidth, pointerAlignment, onSelect }) {
  ...
  return <><div className={`daily-progress-scroll${dense ? " daily-progress-scroll--dense" : ""}`} style={{ paddingTop: `${64 + stackHeight}px`, "--main-marker-size": `${mainMarkerSize}px`, "--replicate-marker-size": `${replicateMarkerSize}px` }} ref={scrollRef}><div className="daily-progress-track" ref={trackRef} style={{ minWidth: dense ? `${Math.max(points.length * 26, 1050)}px` : undefined }}><div className="daily-progress-axis">{completedWidth > 0 && <span className="daily-progress-complete" style={{ width: `${completedWidth}%` }} />}{pulseWidth > 0 && <span className="daily-progress-pulse" style={{ left: `${pulseStart}%`, width: `${pulseWidth}%` }} />}{next && <span className={`daily-next-pointer daily-next-pointer--${pointerAlignment}`} style={{ left: `${next.percent}%`, bottom: `${14 + (next.stackSize - 1) * replicateStep}px` }}><span><strong>{next.status === "pending" ? "Awaiting result" : next.replicate > 1 ? `Next replicate · ${next.time}` : "Next capture"}</strong><small>{relativeFutureTime(next.scheduled_at)}</small></span><i aria-hidden="true" /></span>}{ticks.map(tick => <span className="daily-progress-tick" style={{ left: `${tick.percent}%` }} key={tick.time} />)}{points.map(point => <span className="daily-progress-cluster" style={{ left: `${point.percent}%` }} key={point.scheduled_at}>{point.captures.map(capture => { const label = capture.replicate > 1 ? `Replicate ${capture.replicate}` : "Time point"; const unavailable = capture.status === "succeeded" && !capture.image_available ? " No saved image is available." : ""; const details = `${label} at ${capture.time}: ${capture.status}.${capture.message ? ` ${capture.message}` : ""}${capture.image_available ? " Select to view image." : unavailable}`; const className = `daily-progress-point daily-progress-point--${capture.status}${capture.replicate > 1 ? " daily-progress-point--replicate" : ""}${capture.image_available ? " daily-progress-point--viewable" : ""}`; const style = { top: `${-(capture.replicate - 1) * replicateStep}px` }; return capture.image_available ? <span role="button" className={className} style={style} title={details} aria-label={details} tabIndex="0" onClick={() => onSelect(capture)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect(capture); } }} key={capture.scheduled_at} /> : <span className={className} style={style} title={details} aria-label={details} tabIndex="0" key={capture.scheduled_at} />; })}</span>)}</div><div className="daily-progress-scale">{ticks.map(tick => <span style={{ left: `${tick.percent}%` }} key={tick.time}>{tick.time}</span>)}</div></div></div>{dense && <small className="daily-progress-scroll-hint">Scroll to inspect every time point.</small>}</>;
}
...
function DailyActivity({ schedule: s }) {
  ...
  return <section className="dashboard-grid"><section className="card overview-card activity-card"><div className="section-heading"><div><h3>{s.varies_by_day ? "Imaging windows across day blocks" : "Daily imaging window"}</h3><p>{s.varies_by_day ? `Up to ${s.daily_time_points} time points and ${s.daily_captures} planned captures on a scheduled day.` : `${s.daily_time_points} time points and ${s.daily_captures} planned captures per day.`}</p></div></div><div className="activity-summary"><div><span>Imaging window</span><strong>{activity.window_label}</strong><small>{activity.window_duration_label}</small></div><div><span>Cadence</span><strong>{activity.cadence_label}</strong></div><div><span>Peak load</span><strong>{activity.peak_captures_per_hour} captures / hour</strong></div></div><div className="activity-profile-shell" role="img" aria-label="Hourly imaging activity across scheduled days"><div className="activity-period"><span>00–11</span><div className="activity-hours">{hours(activity.hours.slice(0, 12))}</div></div><div className="activity-period"><span>12–23</span><div className="activity-hours">{hours(activity.hours.slice(12))}</div></div></div><div className="activity-legend"><span>No captures</span><i /><span>Peak: {activity.peak_captures_per_hour} captures per hour</span></div><div className="replicate-compact"><div><strong>Technical replicates</strong><p>{s.replicates} capture{s.replicates === 1 ? "" : "s"} per time point, spaced {s.replicate_interval_seconds} seconds apart.</p></div><div className="replicate-burst">{s.replicate_offsets.map(item => <div key={item.number}><strong>{item.number}</strong><span>+{item.offset_seconds}s</span></div>)}</div></div></section></section>;
}
