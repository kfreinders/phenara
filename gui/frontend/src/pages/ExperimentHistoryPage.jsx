...
export function ExperimentHistoryPage() {
  ...
  return <section className="history-page">
    ...
    <div className="history-list">{experiments.map(run => <article className="card history-row" key={run.run_id}><div><span className={`lifecycle-badge lifecycle-badge--${run.state}`}>{labels[run.state] ?? run.state}</span><h3>{run.name}</h3><p>{run.start_date} → {run.end_date}{run.researcher ? ` · ${run.researcher}` : ""}</p></div><div className="history-outcomes"><strong>{run.capture_summary?.succeeded ?? "—"}</strong><span>successful captures</span>{run.archive_size_bytes != null && <small>Exported ZIP: {formatBytes(run.archive_size_bytes)}</small>}</div><div className="history-state"><strong>{run.data_present ? "Cleanup required" : "Metadata only"}</strong><span>{run.data_present ? "Raw files remain locally" : "Raw files exported and removed"}</span><Link className="button-link secondary" to={`/experiments/${run.run_id}`}>View experiment</Link></div></article>)}</div>
