...
Phenara acquisition system. By default, these images are stored in:

\begin{verbatim}
$PHENARA_ROOT/captures
\end{verbatim}
...
The analysis is executed from the Phenara repository directory using the canopy
...
Phenara relies on PlantCV to separate plant tissue from the background by
...
Phenara analyses plants within predefined regions of interest. Each region of
...
background areas from being included in the trait measurements. While Phenara
...
Next, Phenara needs to know about the expected tray layout. These can be
